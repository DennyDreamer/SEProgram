...
public class LoginController {
    ...
    public Result login(@RequestBody LoginUsers LoginUser, HttpServletRequest request) {
        ...
        boolean IsString = number.matches("\\d+");
        if (IsString == false) {
            return new Result(600);
        } else {

            Integer admin_id = Integer.parseInt(number);

//        userExample example = new userExample();
//
//        userExample.Criteria criteria = new userExample().createCriteria();
//
//        criteria.andUserNumberEqualTo(number);
//
//        System.out.println(number);
//
//        List<user> TheseUser = UserMapper.selectByExample(example);

            Integer id = UserMapper.selectIdByUserNumber(number);//TheseUser.get(0).getUserId();//UserMapper.selectIdByUserNumber(number);
